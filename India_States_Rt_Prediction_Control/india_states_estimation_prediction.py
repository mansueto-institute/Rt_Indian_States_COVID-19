...
for state in states:
    ...
    g=open('covid19-in-india/covid_19_india.csv', 'r')
    reader=csv.reader(g)
...
##### estimation and prediction
    ...
    plt.tight_layout()
    plt.legend()
    str='NewCases_Timeseries_'+state+'.png'
    plt.savefig(str)
    #plt.show()
    plt.clf()
...
#lyyy=np.cumsum(lwy)
    ...
    plt.text(xd[-1]+timedelta(days=2),max(DeltaT)/2., 'today',fontsize=14)
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    str='Observed_Predicted_New_Cases_Control_Rtarget_'+state+'.png'
    plt.savefig(str)
    #plt.show()
